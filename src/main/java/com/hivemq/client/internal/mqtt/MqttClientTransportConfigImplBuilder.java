...
public abstract class MqttClientTransportConfigImplBuilder<B extends MqttClientTransportConfigImplBuilder<B>> {
    ...
    @NotNull MqttClientTransportConfigImpl buildTransportConfig() {
        if (quicConfig != null) {
            if (webSocketConfig != null) {
                throw new IllegalStateException("QUIC transport cannot be combined with WebSocket.");
            }
            if (proxyConfig != null) {
                throw new IllegalStateException("QUIC transport cannot be combined with a proxy.");
            }
        }
